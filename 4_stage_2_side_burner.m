...
global Rocket_drag_coefficient = 0.4		% Might be slightly pessimistic
...
function cost = Simulate_rocket(Rocket_parameters)
  ...
  for stage = 1:Number_of_stages
	  ...
	  if (Stage_max_altitude == 0)
		  printf("This did not fly, it burned!\n");
		  stage = 100;	% Not sure this works to force the loop to stop...
		  break;
	  end
  ...
  % Maximize velocity
  cost = Total_rocket_cost/ (Rocket_max_velocity * Rocket_altitude);
...
function [Stage_max_altitude, Stage_max_accelleration, Stage_max_vertical_velocity, Stage_max_horizontal_velocity, Stage_altitude_at_max_velocity, Stage_time_at_max_velocity] = Simulate_stage(Motor_parameters)
  ...
  % These are ignored when we give fixed timestamps: 'RelTol',0.001, 'AbsTol',.001
  ...
  printf("--------------------------\n");
  if (n == 4)
	  printf("Sorry, this did not even get off the ground properly\n");
	  % If we would continue, there would be serious index out of bounds errors...
  else
	  x = Result(:,1);
	  y = Result(:,2);
	  Vx = Result(:,3);
...
% I just made this up
Rocket_parameters = [30, 3, 20, 2, 10, 1, 1, 0.1]
...
%}
Number_of_stages = 4
Population_initial_range = [0,0,0,0,0,0,0,0 ; 42, 42, 42, 42, 32, 32, 32, 32]
%{
