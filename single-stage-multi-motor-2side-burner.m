...
function cost = Simulate_stage(Motor_parameters)
  ...
  Number_of_motors = round(Motor_parameters(3))
  ...
  if (Motor_length <= 0 || Motor_outside_diameter <= 0 || Number_of_motors <= 0)
	  cost = Inf
	  return
  end
  ...
  while y(n) > 0                  % Run until rocket hits the ground
    ...
    if t(n) <= 0                              % Launch phase 1
     ...
     elseif t(n) < Burn_time            % Launch phase 2: boosting
        Thrust(n) = Thrust_per_motor * Number_of_motors;                          
...
% Small tests
Motor_parameters = [4.9520, 1.1490, 1]     % max altitude: 9827m with 10T payload for 28k dollar           % optimized for minimal cost / (kg of payload * km of altitude)
Motor_parameters = [9.3524, 1.0213, 1]	% max altitude: 17130m with 10T payload for only 42k dollar	% optimized for minimal cost / (kg of payload * km of altitude)

Motor_parameters = [17.2204, 2.4422, 1]	% max altitude: 62865m with 10T payload for 448k dollar		% optimized for altitude
Motor_parameters = [79.2182, 2.4422, 1]	% max altitude: 90701m with 10T payload for 2m dollar		% optimized for altitude

Motor_parameters = [12.2037, 1.8130, 1]	% max. alt. 43km with 10T payload for 175k dollar		& optimized for minimal cost / (kg of payload * km of altitude^2)

Motor_parameters = [21.914, 2.4795, 2]	% max. alt. 72km with 10T payload for 1175k dollar		& optimized for minimal cost / (kg of payload * km of altitude^4)

Simulate_stage(Motor_parameters);

% The real GA
Population_initial_range = [0,0,0 ; 42, 42, 10]
PopulationSize = 42;
...
[solution, cost_of_solution, exitflag, output, population, scores] = ga(@Simulate_stage, 3, [], [], [], [], [], [], [], options)
